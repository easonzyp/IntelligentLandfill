...
import com.google.zxing.client.android.encode.MD5;
import com.google.zxing.client.android.history.HistoryActivity;
import com.google.zxing.client.android.history.HistoryItem;
import com.google.zxing.client.android.history.HistoryManager;
import com.google.zxing.client.android.result.ResultHandler;
import com.google.zxing.client.android.result.ResultHandlerFactory;
import com.google.zxing.client.android.result.ScanResultBean;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback {
    ...
    public void onCreate(Bundle icicle) {
        ...
        bhList = (List<String>) getIntent().getSerializableExtra("bhList");

        tv_light.setOnClickListener(v -> {
            try {
                isTorchOn = !isTorchOn;
                cameraManager.setTorch(isTorchOn);
                tv_light.setText(isTorchOn ? "关灯" : "开灯");
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                ...
                if (shouldShowRequestPermissionRationale(permissions[0])) {
                    ...
                    Toast.makeText(CaptureActivity.this, "请给与app相机权限，否则将无法扫码", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
   ...
   /* @Override
    ...
    private static void drawLine(Canvas canvas, Paint paint, ResultPoint a, ResultPoint
            b, float scaleFactor) {
        if (a != null && b != null) {
...
//        maybeSetClipboard(resultHandler);
        ...
        //TODO 有可能是扫码结果
        if (isBase64(rawResult.toString())) {
            //加密
            String result = MD5.getFromBase64(rawResult.toString());
            decode(result);
        } else {
            //未加密
            decode(rawResult.toString());
        }
    }

    private void decode(String result) {
        try {
            ScanResultBean bean = new Gson().fromJson(result, ScanResultBean.class);
            if (scanType == 1) {
                viewfinderView.setVisibility(View.GONE);
                gotoActivity(bean);
                /*if (bhList == null || bhList.size() == 0) {
                    Toast.makeText(this, "请扫描已被分配的风险点二维码", Toast.LENGTH_LONG).show();
                    return;
                }
                if (bhList.contains(bean.getRiskBH())) {
                    gotoActivity(bean);
                } else {
                    Toast.makeText(this, "请扫描已被分配的风险点二维码", Toast.LENGTH_LONG).show();
                }*/
                ...
                if (riskId.equals(bean.getRiskBH())) {
                    ...
                    Toast.makeText(this, "请扫描该风险点对应的二维码", Toast.LENGTH_LONG).show();
                    delayExecution();
                }
            }
        } catch (Exception e) {
            Toast.makeText(this, "请扫描正确的风险点二维码", Toast.LENGTH_LONG).show();
            delayExecution();
        }
    }
    ...
    private static boolean isJson(String targetStr) {
        if (TextUtils.isEmpty(targetStr)) {
            return false;
        }
        try {
            new Gson().fromJson(targetStr, ScanResultBean.class);
            return true;
    ...
    public void gotoActivity(ScanResultBean bean) {
        ARouter.getInstance().build("/app/RiskPointsDetailActivity")
                .withString("riskName", bean.getRiskName())
                .withString("riskId", bean.getRiskBH())
                .withString("timeStamp", bean.getTimestamp())
                .navigation();
        finish();
    }
